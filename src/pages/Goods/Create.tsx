...
class CreatePage extends Component<any> {
  ...
  onGoodsAttributeValueChange = (goodsAttributeValues:any,goodsAttributeId:any) => {
    ...
    let checkedGoodsAttributeValues:any = this.state.checkedGoodsAttributeValues;
    ...
    let descarteValues:any = this.descartes(checkedGoodsAttributeValues);
    ...
    if(descarteValues.length != 0) {
      ...
      checkedGoodsAttributeValues.map((descarteValue:any,index:any) => {
              ...
              <Form.Item {...formItemLayout} label="所属商家" name={'shop_id'}>
                ...
                >
                  {!!this.state.data.shops && this.state.data.shops.map((option:any) => {
                    return (<Option key={option.id.toString()} value={option.id.toString()}>{option.title}</Option>)
                  })}
                </Select>
              </Form.Item>
                    ...
                    <div style={{marginTop:'20px'}}>
                      {!!this.state.systemGoodsAttributes && this.state.systemGoodsAttributes.map((systemGoodsAttribute:any) => {
                        if(systemGoodsAttribute.style == 1) {
                          // 多选
                          return (
                            <Form.Item {...attrFormItemLayout} label={systemGoodsAttribute.name} name={'system_goods_attribute_'+systemGoodsAttribute.id}>
                              <Checkbox.Group>
              ...
              <Form.Item {...formItemLayout} label="商品规格">
                <div style={{background:'rgba(93,178,255,.1)',border:'1px solid #bce8f1',borderRadius:'2px',padding:'10px'}}>
                    ...
                    {this.state.goodsAttributes ? 
                      <div style={{marginTop:'20px'}}>
                        <Form.Item {...goodsAttributeFormItemLayout} label='选择规格' name={'goodsAttribute'}>
                              <Checkbox.Group
                                onChange={this.onGoodsAttributeChange}
                              >
                              {!!this.state.goodsAttributes && this.state.goodsAttributes.map((goodsAttribute:any) => {
                                  // 多选
                                  return (
                          ...
                          if(this.state.checkedGoodsAttributes.indexOf(goodsAttribute.id) != -1) {
                            return (
                              ...
                              >
                                ...
                                >
                                  {!!goodsAttribute.vname && goodsAttribute.vname.map((option:any) => {
                                    return (<Checkbox value={option.id}>{option.vname}</Checkbox>)
                                  })}
                                </Checkbox.Group>
                              </Form.Item>
